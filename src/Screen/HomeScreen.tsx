...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  return (
    <View style={[styles.container, isDarkMode && styles.darkContainer]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={[styles.weatherCard, isDarkMode && styles.darkWeatherCard]}>
                <Text style={[styles.weatherDate, isDarkMode && styles.darkText]}>{new Date(item.dt_txt).toLocaleDateString()}</Text>
                <Image source={{ uri: `https://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png` }} style={styles.weatherIcon} />
                <Text style={[styles.weatherTemp, isDarkMode && styles.darkText]}>{item.main.temp.toFixed(1)}°C</Text>
                <Text style={[styles.weatherDesc, isDarkMode && styles.darkText]}>{item.weather[0].description}</Text>
              </View>
            )}
          />
        )}
      ...
      <TouchableOpacity style={[styles.chatButton, isDarkMode && styles.darkChatButton]} onPress={() => navigation.navigate("Chat")}>
        <Text style={[styles.chatButtonText, isDarkMode && styles.darkText]}>Chat en tiempo real</Text>
      </TouchableOpacity>
